...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, verbose=1, render=False, num_maps=None, seed=None, num_stacked_img=1):
        ...
        if self.num_maps:
            self.prepare_maps(self.num_maps)
    ...
    def prepare_maps(self, num_maps):
        self.road_storage = []
        self.world_storage = []
        ...
        for i in range(num_maps):
            ...
            self.road_poly = []
            while True:
                success, maps_info = self._create_track_my()
                new_map = success
                if success:
                    self.road_storage.append(maps_info[0])
                    self.world_storage.append(maps_info[1])
                    self.road_poly_storage.append(maps_info[2])
                    self.track_storage.append(maps_info[3])
                    break
                if self.verbose == 1:
                    print(
